...
class TestDecodeStakeInfo(unittest.TestCase):
    ...
    def test_decode_vec_matches_python_impl(self):
        stake_info_list: List[bt_decode.StakeInfo] = (
            bt_decode.StakeInfo.decode_vec(
                bytes.fromhex(TEST_STAKE_INFO_HEX["vec normal"])
            )
        )

        stake_info_py_list: List[bittensor.StakeInfo] = bittensor.StakeInfo.list_from_vec_u8(
        ...
        for stake_info, stake_info_py in zip(stake_info_list, stake_info_py_list):
            for attr in dir(stake_info):
                if not attr.startswith("__") and not callable(getattr(stake_info, attr)):
    ...
    def test_decode_vec_vec_matches_python_impl(self):
        stake_info_list: List[Tuple[str, List[bt_decode.StakeInfo]]] = (
        ...
        stake_info_py_dict: Dict[str, List[bittensor.StakeInfo]] = bittensor.StakeInfo.list_of_tuple_from_vec_u8(
            list(bytes.fromhex(TEST_STAKE_INFO_HEX["vec vec normal"]))
        )

        for stake_info_tuple, (coldkey, stake_info_py_list) in zip(stake_info_list, stake_info_py_dict.items()):
